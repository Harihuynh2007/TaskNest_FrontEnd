// src/components/Card/Due/DueDatePopup.jsx
import React, { useRef, useEffect, useState } from 'react';
import FocusLock from 'react-focus-lock';
import { motion, AnimatePresence } from 'framer-motion';
import { format } from 'date-fns';
import DueDatePicker from './DueDatePicker';
import useDueDateForm from './useDueDateForm';
import {
  PopupContainer,
  Arrow,
  PopupHeader,
  PopupContent,
  FieldGroup,
  Label,
  Select,
  ButtonRow,
  SaveButton,
  RemoveButton,
  QuickButtonsRow,
  QuickBtn,
} from './DueDatePopup.styles';

export default function DueDatePopup({ card, onClose, onUpdated, anchorRect }) {
  const ref = useRef(null);
  const [mounted, setMounted] = useState(false);

  // sử dụng hook form tách riêng
  const {
    startDate,
    dueDate,
    dueTime,
    reminder,
    recurrence,
    setStartDate,
    setDueDate,
    setDueTime,
    setReminder,
    setRecurrence,
    save,
    remove,
    quickSelect,
    loading,
    error,
  } = useDueDateForm(card, onUpdated);

  useEffect(() => {
    if (anchorRect) setMounted(true);
  }, [anchorRect]);

  // click outside (đã giữ)
  useEffect(() => {
    const handleClick = (e) => {
      if (ref.current && !ref.current.contains(e.target)) onClose();
    };
    document.addEventListener('mousedown', handleClick);
    return () => document.removeEventListener('mousedown', handleClick);
  }, [onClose]);

  // ESC close
  useEffect(() => {
    const handleKey = (e) => e.key === 'Escape' && onClose();
    document.addEventListener('keydown', handleKey);
    return () => document.removeEventListener('keydown', handleKey);
  }, [onClose]);

  if (!anchorRect || !mounted) return null;

  return (
    <AnimatePresence>
      <FocusLock>
        <motion.div
          key="duedate-popup"
          initial={{ opacity: 0, scale: 0.96, y: -6 }}
          animate={{ opacity: 1, scale: 1, y: 0 }}
          exit={{ opacity: 0, scale: 0.96, y: -6 }}
          transition={{ duration: 0.16, ease: 'easeOut' }}
        >
          <PopupContainer
            ref={ref}
            anchorRect={anchorRect}
            role="dialog"
            aria-label="Set due date"
            aria-busy={loading}
          >
            <Arrow anchorRect={anchorRect} />

            {/* Header hiển thị trạng thái */}
            <PopupHeader>
              {card?.due_date && new Date(card.due_date) < new Date() && (
                <div
                  style={{
                    color: '#f87171',
                    fontSize: 12,
                    marginBottom: 6,
                    fontWeight: 500,
                  }}
                >
                  ⚠ Overdue by{' '}
                  {Math.max(
                    1,
                    Math.floor(
                      (new Date() - new Date(card.due_date)) / (1000 * 60 * 60 * 24)
                    )
                  )}{' '}
                  {Math.floor(
                    (new Date() - new Date(card.due_date)) / (1000 * 60 * 60 * 24)
                  ) > 1
                    ? 'days'
                    : 'day'}
                </div>
              )}
              Dates{' '}
              {dueDate && (
                <span style={{ color: '#60a5fa', fontSize: 13, marginLeft: 6 }}>
                  ({format(new Date(`${dueDate}T${dueTime || '00:00'}:00Z`), 'dd MMM, HH:mm')})
                </span>
              )}
            </PopupHeader>

            {/* Nội dung form */}
            <PopupContent>
              <FieldGroup>
                <DueDatePicker
                  label="Start date"
                  value={startDate}
                  onChange={setStartDate}
                  withTime={false}
                />
              </FieldGroup>

              <FieldGroup>
                <DueDatePicker
                  label="Due date"
                  value={
                    dueDate
                      ? new Date(`${dueDate}T${dueTime || '00:00'}:00Z`).toISOString()
                      : null
                  }
                  onChange={(v) => {
                    const d = new Date(v);
                    setDueDate(format(d, 'yyyy-MM-dd'));
                    setDueTime(format(d, 'HH:mm'));
                  }}
                  withTime
                />
              </FieldGroup>

              {/* Quick set */}
              <QuickButtonsRow>
                <QuickBtn onClick={() => quickSelect(0)} disabled={loading}>
                  Today
                </QuickBtn>
                <QuickBtn onClick={() => quickSelect(1)} disabled={loading}>
                  Tomorrow
                </QuickBtn>
                <QuickBtn onClick={() => quickSelect(7)} disabled={loading}>
                  Next week
                </QuickBtn>
              </QuickButtonsRow>

              {/* Reminder */}
              <FieldGroup>
                <Label>Reminder</Label>
                <Select
                  value={reminder}
                  onChange={(e) => setReminder(e.target.value)}
                  disabled={loading}
                >
                  <option value="0">At due time</option>
                  <option value="5">5 minutes before</option>
                  <option value="10">10 minutes before</option>
                  <option value="60">1 hour before</option>
                  <option value="1440">1 day before</option>
                </Select>
              </FieldGroup>

              {/* Repeat */}
              <FieldGroup>
                <Label>Repeat</Label>
                <Select
                  value={recurrence}
                  onChange={(e) => setRecurrence(e.target.value)}
                  disabled={loading}
                >
                  <option value="never">Never</option>
                  <option value="daily">Daily</option>
                  <option value="weekly">Weekly</option>
                  <option value="monthly">Monthly</option>
                </Select>
              </FieldGroup>

              {/* Error message */}
              {error && (
                <div
                  style={{
                    color: '#f87171',
                    fontSize: 12,
                    marginTop: 4,
                    textAlign: 'center',
                  }}
                >
                  {error}
                </div>
              )}

              {/* Buttons */}
              <ButtonRow>
                <SaveButton onClick={save} disabled={loading}>
                  {loading ? 'Saving…' : 'Save'}
                </SaveButton>
                <RemoveButton onClick={remove} disabled={loading}>
                  Remove
                </RemoveButton>
              </ButtonRow>
            </PopupContent>
          </PopupContainer>
        </motion.div>
      </FocusLock>
    </AnimatePresence>
  );
}
